...
public class TimelineController {
 private final TimelineService service; private final ZoneId timeZone; public TimelineController(TimelineService service,@Value("${morrow.time-zone:Asia/Seoul}")String timeZone){this.service=service;this.timeZone=ZoneId.of(timeZone);}
 @PatchMapping("/{id}") ResponseEntity<TimelineResponse> update(@PathVariable UUID id,@Valid @RequestBody UpdateRequest request){var updated=service.update(id,new TimelineService.UpdateTimeline(request.title(),request.detail(),request.userConfirmed()));return ResponseEntity.ok(TimelineResponse.from(updated,timeZone));}
