...
public class RecommendationController {
 private final RecommendationService service; private final RequestUserResolver users; public RecommendationController(RecommendationService service,RequestUserResolver users){this.service=service;this.users=users;}
 @PostMapping("/{id}/feedback") ResponseEntity<FeedbackResponse> createFeedback(@PathVariable UUID id,@Valid @RequestBody FeedbackRequest request){var feedback=service.createFeedback(id,users.resolve("default-user"),new RecommendationService.CreateFeedback(request.completed(),request.helpful(),request.note()));return ResponseEntity.created(URI.create("/api/v1/recommendations/"+id+"/feedback/"+feedback.getId())).body(FeedbackResponse.from(feedback));}
