...
public class DashboardController {
 ...
 @GetMapping("/dashboard") DashboardResponse dashboard(@RequestParam(defaultValue="default-user")String userId){var data=service.getDashboard(users.resolve(userId));return new DashboardResponse(data.wellnessLoad(),data.score(),MetricsResponse.from(data.metrics()),data.timelines().stream().map(value->TimelineResponse.from(value,timeZone)).collect(Collectors.toList()),data.recommendation()!=null?RecommendationResponse.from(data.recommendation()):null,"의료 진단이 아닌 일상 웰니스 분석입니다.");}
