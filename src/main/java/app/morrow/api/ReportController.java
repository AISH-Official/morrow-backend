...
public class ReportController {
 ...
 record WeeklyReportResponse(int totalCheckIns,String topStatus,String topCause,double improvementRate,double changeFromPrevious,List<DailyPointResponse> dailyScores,List<String> patterns,String insights){static WeeklyReportResponse from(ReportService.WeeklyReport r){return new WeeklyReportResponse(r.totalCheckIns(),r.topStatus(),r.topCause(),r.improvementRate(),r.changeFromPrevious(),r.dailyScores().stream().map(DailyPointResponse::from).toList(),r.patterns(),r.insights());}}
}
