...
public class DashboardService {
 private final CheckInRepository checkInRepository; private final TimelineService timelineService; private final RecommendationService recommendationService; private final HealthSignalSnapshotService healthSnapshots; private final RecoveryScoreCalculator recoveryScores;
 public DashboardService(CheckInRepository checkInRepository,TimelineService timelineService,RecommendationService recommendationService,HealthSignalSnapshotService healthSnapshots,RecoveryScoreCalculator recoveryScores){this.checkInRepository=checkInRepository;this.timelineService=timelineService;this.recommendationService=recommendationService;this.healthSnapshots=healthSnapshots;this.recoveryScores=recoveryScores;}
 public DashboardData getDashboard(String userId){var todayStart=OffsetDateTime.now().withHour(0).withMinute(0).withSecond(0).withNano(0);var recentCheckIns=checkInRepository.findByUserIdAndRecordedAtAfterOrderByRecordedAtDesc(userId,todayStart);var score=recoveryScores.calculate(scoreSnapshot(userId),recentCheckIns);var wellnessLoad=recoveryScores.wellnessLoad(score);var metrics=calculateMetrics(userId);var timelines=timelineService.findRecentByUserId(userId,todayStart);var activeRecommendation=recommendationService.findActiveByUserId(userId).orElse(null);return new DashboardData(wellnessLoad,score,metrics,timelines,activeRecommendation);}
